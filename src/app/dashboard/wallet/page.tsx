...
function UniversalWalletPage() {
    ...
    const securityScore = useMemo(() => {
        ...
        if (user?.beneficiaryId) score += 15;
        return score;
    }, [user]);
            ...
            <main className="max-w-7xl mx-auto px-4 py-12">
                ...
                <Tabs defaultValue="overview" className="space-y-10">
                    <TabsList className="bg-white/5 border border-white/5 p-1.5 rounded-[1.5rem] h-16 w-full max-w-2xl mx-auto flex">
                        ...
                        <TabsTrigger value="heritage" className="flex-1 rounded-xl font-black uppercase italic text-[10px] data-[state=active]:bg-accent data-[state=active]:text-black transition-all"><HeartPulse size={14} className="mr-2" /> Héritage</TabsTrigger>
                        <TabsTrigger value="rankings" className="flex-1 rounded-xl font-black uppercase italic text-[10px] data-[state=active]:bg-accent data-[state=active]:text-black transition-all"><BarChart3 size={14} className="mr-2" /> Wealth</TabsTrigger>
                    </TabsList>
                                ...
                                <div className="space-y-6">
                                    <div className="p-6 bg-primary/10 border border-primary/20 rounded-3xl">
                                        <div className="flex justify-between items-center mb-4">
                                            <span className="text-[10px] font-black uppercase text-primary tracking-widest">Rendement Annuel (APR)</span>
                                            <Badge className="bg-primary text-white font-black italic">{stats.apr}% APR</Badge>
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        <Label className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-1">Montant à Staker</Label>
                                        <div className="relative">
                                            ...
                                            <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-muted-foreground uppercase">DKST</div>
                                        </div>
                                    </div>
                                    <Button onClick={() => secureAction(handleStake)} disabled={isProcessingAction || !stakeAmount} className="w-full h-16 bg-primary text-white font-black uppercase italic rounded-2xl shadow-xl shadow-primary/20">
                                        {isProcessingAction ? <Loader2 className="animate-spin" /> : "Activer le Contrat de Staking"}
                                    </Button>
                                </div>
                            </Card>
                            ...
                            <Card className="bg-black/40 border border-white/5 rounded-[3rem] p-10 flex flex-col justify-between">
                                <div className="space-y-8">
                                    <div className="flex justify-between items-start">
                                        <div><h3 className="text-xl font-black uppercase italic">Ma Position Vault</h3><p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Actifs sous gestion</p></div>
                                        <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-primary"><Timer size={24}/></div>
                                    </div>
                                    <div className="space-y-6">
                                        <div>
                                            <p className="text-[9px] font-black uppercase text-white/40 mb-1">Capital Bloqué</p>
                                            <p className="text-4xl font-black text-white italic">{user?.stakedBalance?.toFixed(2) || 0} <span className="text-sm not-italic opacity-40">DKST</span></p>
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black uppercase text-white/40 mb-1">Récompenses accumulées (Live)</p>
                                            <p className="text-4xl font-black text-green-400 italic">+{stats.stakingRewards.toFixed(6)}</p>
                                        </div>
                                    </div>
                                </div>
                                <Button onClick={() => secureAction(handleUnstake)} disabled={isProcessingAction || !user?.stakedBalance} variant="outline" className="w-full h-16 border-white/10 rounded-2xl font-black uppercase italic mt-10 hover:bg-white/5">
                                    {isProcessingAction ? <Loader2 className="animate-spin" /> : "Retirer Capital & Intérêts"}
                                </Button>
                            </Card>
                        </div>
                    </TabsContent>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                            <Card className="lg:col-span-5 glossy-card border-none rounded-[3rem] p-10 space-y-8 relative overflow-hidden">
                                ...
                                <div className="relative z-10 space-y-8">
                                    ...
                                    <p className="text-sm text-muted-foreground leading-relaxed italic">"Désignez un bénéficiaire pour vos actifs GCV. Si aucune activité n'est détectée durant la période choisie, vos jetons lui seront transmis automatiquement."</p>
                            ...
                            <Card className="lg:col-span-7 bg-black/40 border border-white/5 rounded-[3rem] p-10 flex flex-col justify-between relative overflow-hidden">
                                <div className="absolute -bottom-10 -right-10 p-12 opacity-5"><Activity size={200} /></div>
                                <div className="space-y-10 relative z-10">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/5 space-y-4">
                                            ...
                                            <div className="flex items-end gap-3">
                                                ...
                                                <div className="w-2 h-2 rounded-full bg-green-500 animate-ping mb-2" />
                                            </div>
                                        </div>
                                        <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/5 space-y-4">
                                            <p className="text-[10px] font-black uppercase text-white/40 tracking-widest">Bénéficiaire Désigné</p>
                                    ...
                                    <div className="p-8 bg-accent/5 rounded-[2.5rem] border border-accent/10 space-y-6">
                                        <div className="flex justify-between items-end">
                                            ...
                                            <span className="text-xs font-bold text-white/40">0 / {user?.heritageThresholdDays || 90} jours</span>
                                        </div>
                                        <Progress value={0} className="h-3 bg-white/5" indicatorClassName="bg-accent" />
                                        <p className="text-[9px] text-muted-foreground uppercase font-bold italic text-center">"Votre heartbeat est actif. La fortune GCV est sous votre contrôle exclusif."</p>
                                    </div>
                                </div>
                            </Card>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            <Card className="lg:col-span-8 glossy-card border-none rounded-[3rem] overflow-hidden">
                                ...
                                <div className="divide-y divide-white/5">
                                    ...
                                    ) : topWealthyUsers?.map((u: any, i) => {
                                        const totalTokens = (u.tokenBalance || 0) + (u.stakedBalance || 0);
                                        ...
                                        return (
                                            <div key={u.id} className={cn("p-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors", isMe && "bg-accent/5 border-l-4 border-l-accent")}>
                                                ...
                                                <div className="text-right">
                                                    <p className="text-2xl font-black text-white italic">${(wealth / 1000000).toFixed(1)}M</p>
                                                    <p className="text-[9px] font-bold text-muted-foreground uppercase">{totalTokens.toFixed(2)} DKST</p>
                                                </div>
                                            </div>
                                        );
                                    })}
